...
@RestController
@RequestMapping("/api/roboregcore-kafka")
public class RoboregcoreKafkaResource {

    private final Logger log = LoggerFactory.getLogger(RoboregcoreKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public RoboregcoreKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
