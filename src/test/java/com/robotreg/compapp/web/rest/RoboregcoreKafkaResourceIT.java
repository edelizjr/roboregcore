...
import com.robotreg.compapp.config.EmbeddedKafka;
import com.robotreg.compapp.config.KafkaSseConsumer;
import com.robotreg.compapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class RoboregcoreKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/roboregcore-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/roboregcore-kafka/consume")
